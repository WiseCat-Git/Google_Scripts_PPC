...
var campaignNameDoesNotContain = ["Display"];
...
var spreadsheetUrl = "https://docs.google.com/spreadsheets/d/1DLIVFKr0_CHP4DnKsXOrdIJo6RQ_2HwaDSR_YkwWHAg/edit#gid=0";
// The URL of the Google Doc the results will be put into.

var textOfInterest = ["crunchy", "rollers"];
...
function addToOutput(dataObject, printlineStart, output, format, formatting, statColumns, calculatedStats) {
  ...
  if (dataObject["Query Count"] < queryCountThreshold) {return;}
  if (dataObject["Impressions"] < impressionThreshold) {return;}
  if (dataObject["Clicks"] < clickThreshold) {return;}
  if (dataObject["Cost"] < costThreshold) {return;}
  if (dataObject["Conversions"] < conversionThreshold) {return;}
    ...
    try {
      ...
      if (formatArray.length == 0) {
        ...
        if (outputArray[0][2] == "Ad Group") {
          sortByColumns.push({column: 3, ascending: true});
        }
